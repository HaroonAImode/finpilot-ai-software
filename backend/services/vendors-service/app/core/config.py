...
class Settings(BaseSettings):
    ...
    # Spend and this vendor's invoices are *derived* from Invoice Service
    # rather than stored here — a cached aggregate would drift the moment an
    # invoice is corrected, and there is no event bus to keep it honest.
    # That makes this a hard dependency for two endpoints, and the code
    # degrades explicitly when it is unreachable rather than silently
    # reporting a vendor's spend as zero.
    invoice_service_url: str = Field(default="http://invoice-service:8002", alias="INVOICE_SERVICE_URL")
    # Short: these are plain aggregate/list queries with no OCR behind them,
    # unlike the scanner hand-off that needs minutes.
    invoice_service_timeout_seconds: float = Field(
        default=15.0, alias="INVOICE_SERVICE_TIMEOUT_SECONDS"
    )
        ...
        if self.is_production:
            if self.trust_company_header:
                raise ValueError(
                    ...
                    "caller read or edit another company's vendors."
                )
