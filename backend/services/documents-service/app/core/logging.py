...
email-connector/app/core/logging.py). Invoice content itself (line items,
vendor names, totals) is business data, not a secret, so it is not
redacted here the way OAuth tokens are elsewhere — only credential-shaped
values are.
"""
import logging
import re
