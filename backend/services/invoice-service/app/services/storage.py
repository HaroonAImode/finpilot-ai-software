...
class StorageManager:
    """Stores uploaded invoice files in S3-compatible storage and computes
    SHA-256 for Rule 7.1's duplicate check. Same shape as
    email-connector/slack-connector's DownloadManager."""
    ...
    async def store_bytes(self, key_id: str, filename: str, content: bytes) -> tuple[str, str]:
        ...
        Invoice row's own UUID), never anything derived from the upload
        itself — matches every other storage manager in this codebase.
        """
        try:
