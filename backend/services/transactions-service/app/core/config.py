...
class Settings(BaseSettings):
    ...
    # scanned-sales ledger (Revenue Manager), never duplicated into a table
    # here. The Dashboard's revenue-side KPIs and the revenue half of both
    # trend charts are read from there on every request. A failure degrades
    # those specific figures (see KpiSet.revenue_unavailable) rather than
    # failing the whole dashboard — expenses are this service's own data and
    # stay correct even if Invoice Service is briefly unreachable.
    invoice_service_url: str = Field(default="http://invoice-service:8002", alias="INVOICE_SERVICE_URL")
    invoice_service_timeout_seconds: float = Field(default=15.0, alias="INVOICE_SERVICE_TIMEOUT_SECONDS")
        ...
        if self.is_production:
            if self.trust_company_header:
                raise ValueError(
                    ...
                    "caller read or edit another company's expenses."
                )
