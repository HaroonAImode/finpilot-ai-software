...
The current branch contains the frontend, backend, architecture report, implementation plans,
research notes, and this handoff. Use `git log -1 --oneline --decorate` to identify the exact
commit being tested; do not hard-code a stale commit hash in a handoff report.

Research, audit, planning, and handoff documents belong under `docs/` and may be committed when
they are part of the project's implementation context. Only secrets, real `.env` files, private
receipt data, generated uploads, and machine-local runtime files belong outside the repository.
`.gitignore` is intentionally tracked and must remain tracked.
...
- Added and updated documentation under `docs/`.

Do not infer that a service or document is missing from a commit subject alone. Check the current
tree and `backend/infra/docker-compose.yml` before making changes.
